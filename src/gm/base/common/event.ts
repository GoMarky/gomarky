import { once as onceFn } from '@/gm/base/common/functional';
import {
  combinedDisposable,
  Disposable,
  DisposableStore,
...
import { LinkedList } from '@/gm/base/common/linkedList';
...
export class GlobalEvent<T = any> extends Disposable {
  protected cancellable = false;

  public preventReason: T | undefined;
  ...
  public preventDefault(reason?: T): void {
    if (this.cancellable) {
      return;
    }
...
export namespace Event {
  ...
  export function bumper<T>(event: Event<T>, nextTick = false, _buffer: T[] = []): Event<T> {
  ...
  class ChainableEvent<T> implements IChainableEvent<T> {
    ...
    public reduce<R>(merge: (last: R | undefined, event: T) => R, initial?: R): IChainableEvent<R> {
  ...
  export interface NodeEventEmitter {
    on(event: string | symbol, listener: FunctionLike): this;

    removeListener(event: string | symbol, listener: FunctionLike): this;
  }
  ...
  export interface DOMEventEmitter {
    addEventListener(event: string | symbol, listener: FunctionLike): void;

    removeEventListener(event: string | symbol, listener: FunctionLike): void;
  }
...
export function toPromise<T>(event: Event<T>): Promise<T> {
  return new Promise(c => Event.once(event)(c));
}
...
class LeakageMonitor {
  ...
  public check(listenerCount: number): undefined | (() => void) {
...
export class Emitter<T> {
  private static readonly _noop = () => undefined;
...
export class AsyncEmitter<T extends IWaitUntil> extends Emitter<T> {
  ...
  async fireAsync(
    eventFn: (thenables: Promise<any>[], listener: FunctionLike) => T
  ): Promise<void> {
    if (!this._listeners) {
      return;
    }
...
export class EventBufferer {
  ...
  public bufferEvents<R = void>(fn: () => R): R {
    const buffer: (() => R)[] = [];
